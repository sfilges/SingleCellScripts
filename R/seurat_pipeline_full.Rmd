...
seurat_object_save <- "./Data/seurat_object.Rdata"
cds_data <- './Data/cds_monocle2.Rdata'
gene_expression <- './Data/monocle2_DEG_genes.csv'
...
adipo_markers <- read_delim(
  file = "MSigDB/hallmark_genes_upregulated_during_adipogenesis.txt",
  delim = "\t",
  col_names = c("gene")
)

hm_seurat <- Seurat::DoHeatmap(
  object = merged_data,
...
#data <- merged_data[adipo_markers$gene,]

data <- tidyr::spread(hm_seurat$data, Feature, Expression)
row.names(data) <- data$Cell

annot <- data.frame(group = data$Identity)
row.names(annot) <- data$Cell

data <- data %>% dplyr::select(-c(Cell, Identity))

write_csv2(data, "./Data/adipogenesis_markers.csv")


hm <- pheatmap::pheatmap(
  mat = t(as.matrix(data)), 
  angle_col = 45,
  scale = 'none',annotation_colors =  
  annotation_col = annot,
  color = viridis::plasma(n = 10),
...
```{r monocle2-DDTree, eval=FALSE, message=FALSE}
...
scedata <- monocle::reduceDimension(
  ...
  norm_method = 'log',
  #num_dim = 3,
  #residualModelFormulaStr = "~nCount_RNA",
...
scedata_ordering_genes <- row.names(clustering_DEG_genes)[order(clustering_DEG_genes$qval)][1:500]
...
write_csv2(data.frame(gene=scedata_ordering_genes), "./Data/ordering_genes.csv")
...
```{r monocle2-plots, eval=FALSE, message=FALSE}
...
```{r monocle2-plots3, eval=FALSE, message=FALSE}
to_be_tested <- row.names(
  clustering_DEG_genes)[order(clustering_DEG_genes$qval)][1:100]
...
#monocle::plot_genes_in_pseudotime(cds_subset, color_by = "group")
```
